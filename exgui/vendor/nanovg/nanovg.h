...
class NVGhandleAllocatorFixed {
	static_assert(_capacity > 0 && "_capacity is 0!");
	static_assert(_capacity <= std::numeric_limits<NVGhandle::_half_type>::max(), "�apacity doesn't fit into handle index type");
...
public:
	...
	void eval(const NVGcmdBuf& buf, NVGcmdArgBuffer& arguments,
		const void* data = nullptr, const NVGcmdLayout* layout = nullptr);
